...
namespace Services.Battle
{
    public class ArcherPhaseService : IArcherPhaseService
    {
            ...
            bool isArmy1)
        {
            ...
            if (isArmy1)
            {
                ...
                for (int i = frontIndex - 1; i >= 0; i--)
                {
                    ...
                    {
                        anyArcher = true;
                        totalScore += ProcessShot(army, enemy, archer, i, true);
                    }
                }
            }
            else
            {
                ...
                for (int i = frontIndex + 1; i < army.Units.Count; i++)
                {
                    ...
                    {
                        anyArcher = true;
                        totalScore += ProcessShot(army, enemy, archer, i, false);
                    }
                }
            }
            ...
            bool isArmy1)
        {
            ...
            // считаем живых между лучником и фронтом своей армии 
            int aliveBetween = 0;
            if (isArmy1)
            {
            ...
            // цель - первый живой враг 
            IUnit? target = null;
            if (isArmy1)
            {
                target = enemy.Units.FirstOrDefault(u => u.IsAlive);
            }
            else
            {
                target = enemy.Units.LastOrDefault(u => u.IsAlive);
            }
            ...
            // проверка на возможность попадания
            ...
            int damage = System.Math.Max(0, archer.Attack - target.Defence);
            target.TakeDamage(damage);

            _logger.LogArcherHit(archer, target, oldHp, target.Health, isArmy1);
